<?php
/**
 * Quote Renewal Controller
 * Handles quote renewal with parent reference and item updates
 */
...
// Check if quote can be renewed (must be APPROVED or REJECTED)
if (!in_array($originalQuote['status'], ['APPROVED', 'REJECTED'])) {
    $_SESSION['error_message'] = __('quote_cannot_be_renewed');
    redirect(url('quotes', 'view', ['id' => $originalQuoteId]));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validateCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        // Create renewal quote if no errors
        if (empty($errors)) {
            ...
            $newQuoteId = $model->createRenewalQuote($originalQuoteId, $quoteData, $validItems);

            if ($newQuoteId) {
                $_SESSION['success_message'] = __('renewal_quote_created_successfully', ['number' => $quoteNumber]);
                redirect(url('quotes', 'view', ['id' => $newQuoteId]));
            } else {
                $errors[] = __('error_creating_renewal_quote');
            }
        }
    }
}
...
// Prepare items with current prices and tax rates
$items = [];
